...
public class PlayerMovement : MonoBehaviour {
    ...
    private String lastWall, actualWall;
    public float wallrunForce, maxWallrunTime, maxWallSpeed;
    private float WallRunTime;
    private bool isWallRight, isWallLeft;
    private bool isWallRunning;
    public float maxWallRunCameraTilt, wallRunCameraTilt;
    private bool ResetDoubleJump = true;
    ...
    private void Jump()
    {
        ...
        if (!grounded)
        {
            ...
            //Disable dashForceCounter if doublejumping while dashing (no dash implemented)
    ...
    private void StartWallRun()
    {
        if(actualWall != lastWall)
        {
            ...
            if (rb.velocity.magnitude <= maxWallSpeed)
            {
                ...
                //To make sure chararcter sticks to wall
                if (isWallRight)
    ...
    private void CheckForWall()
    {
        ...
        if (isWallRight)
        {
            ...
            {
                actualWall = hit.collider.gameObject.name;
            }
        }
        if (isWallLeft)
        {
        ...
        { 
            ...
            textBox.text = Mathf.Round(WallRunTime).ToString();

            if (ResetDoubleJump == true)
            {
                doubleJumpsLeft = startDoubleJumps;  //reset double jump
                ResetDoubleJump = false;
            }
            if (WallRunTime <= 0)
            {
                StopWallRun();//stop wallrunning when max time exceeded
            }
            lastWall = actualWall; //update last wall

        }
        else
        {
            ResetDoubleJump = true;
            WallRunTime = maxWallrunTime;//reset timer when no wallruning
            textBox.text = Mathf.Round(WallRunTime).ToString();//display timer
        }
        //leave wall run
